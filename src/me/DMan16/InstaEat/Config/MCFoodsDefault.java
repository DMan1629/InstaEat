...
public enum MCFoodsDefault {
	...
	CAKE(Material.CAKE,2,0.4F,null,0);
	...
	private static List<Material> extraFood = Arrays.asList(Material.GLISTERING_MELON_SLICE);
	private static List<Material> placeableFood = Arrays.asList(Material.CAKE);
	private static List<Material> notFood = Arrays.asList(Material.CHORUS_FRUIT);
	...
	public static boolean isNotFood(Material material) {
		return notFood.contains(material);
	}
	
	public static boolean isPlaceableFood(Material material) {
		return placeableFood.contains(material);
	}
	
	public static boolean isVanillaFood(Material material) {
		return material.isEdible() && !isNotFood(material) && !isPlaceableFood(material);
	}
	...
	public static boolean isExtraFood(Material material) {
		return material == Material.GLISTERING_MELON_SLICE;
	}
	...
	public static List<Material> getVanillaFood() {
		List<Material> vanilla = new ArrayList<Material>();
		for (MCFoodsDefault food : values()) {
			if (isVanillaFood(food.material)) {
				vanilla.add(food.material);
			}
		}
		return vanilla;
	}
	...
	public static List<Material> getFoods() {
		...
		all.addAll(getVanillaFood());
		all.addAll(getExtraFood());
		return all;
	}
	...
	public static List<Material> getPlaceableFood() {
		return placeableFood;
	}
	
	public static List<Material> getDefaultConfigFood() {
		List<Material> defaultConfigFood = new ArrayList<Material>();
		defaultConfigFood.addAll(getVanillaFood());
		defaultConfigFood.addAll(getExtraFood());
		return defaultConfigFood;
	}
	
	public CustomFood asFood() {
		return new CustomFood(new ItemStack(material),hunger,saturation,chance,effects);
	}
}